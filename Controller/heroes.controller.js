...
const { heroes } = require('../Model/heroes.model.js');
...
exports.findOne = (req, res) => {
    ...
    Heroe.findById(id).then(data => {
        if(!data){
            res.status(404).send({ message: `No se encontro el elemento con el id: ${id}` });
        }else {
            res.send(data);
        }
    }).catch( err => {
...
exports.delete = (req,res) => {
    ...
    Heroe.findByIdAndUpdate(id, { activo: false }, { useFindAndModify: false }).then(data => {
        if(!data){
            res.status(404).send({
                message: `No se pudo actualizar el heroe con el if: ${id}`
            });
        }else{
            res.send({
